...
app.secret_key = b'\xc2\xc4\xa1}|\x9cbwP\x1exn\xbfW\xeb\x1a\xe8\xc2\x8e\x88\xff2J8'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['SECRET_KEY'] = b'\xc2\xc4\xa1}|\x9cbwP\x1exn\xbfW\xeb\x1a\xe8\xc2\x8e\x88\xff2J8'
...
class Stats(db.Model):
    ...
    def __repr__(self):
        return f'''Stats("{self.overall_feeling}", "{self.time_slept}", "{self.worked_out}", 
                        "{self.ate_healthy}", "{self.time_worked_out}", "{self.workout_type}", 
                        "{self.unhealthy_food}", "{self.proud_achievement}")'''
...
def log_data():
    ...
    if request.method == 'POST':
        ...
        time_worked_out = 0  # Default Value - If User did not Work out
        workout_type = "None"  # Default Value - If User did not Work out
